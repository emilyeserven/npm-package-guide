...
WordPress Site (with ACF)
    |
    v
Schema Discovery Script  --------►  wp-schema.json (committed to repo)
    |
    v
Sample Data Script  --------------►  wp-sample-data.json (committed to repo)
    |
    v
Claude Code Web Session  ---------►  TypeScript interfaces + mock factories + fixtures
    |
    v
Your Frontend Project  -----------►  Type-safe components with tested data layer
```
...
<SectionList>
<ColItem>In wp-admin, go to **Users → Add New User**</ColItem>
...
<SectionList>
<ColItem>Go to **Users → Edit** for `api-bot`</ColItem>
...
<ColItem>Store it securely — you'll use it locally only, never commit it</ColItem>
</SectionList>

**1c. Enable ACF Fields in REST API**

<SectionList>
<ColItem>Go to **Custom Fields → Field Groups**</ColItem>
...
<SectionList>
<ColItem>**Custom Fields → Post Types** → edit each type</ColItem>
<ColItem>Enable **Advanced Configuration** → click the **REST API** tab</ColItem>
 ...
 *
 * Setup: No dependencies required — uses native Node.js fetch (Node 18+).
 */
...
async function discoverPostTypeSchemas(root) {
  log("📋 Discovering post type schemas (OPTIONS requests)...");
  const routes = root.routes || {};
  const typeRoutes = [];
...
async function discoverACFFieldGroups() {
  log("🔧 Attempting ACF field group discovery...");
  ...
  if (acfIndex._error) {
    log("   ⚠️  ACF REST API (acf/v3) not available or not authenticated.");
    log(
  ...
  if (fieldGroups._error) {
    log("   ⚠️  Could not fetch ACF field groups (may need authentication).");
...
async function discoverAngieACF() {
  log("🔧 Checking for angie-acf-mcp plugin endpoints...");
...
async function discoverTaxonomySchemas() {
  log("📂 Discovering taxonomy schemas...");
...
async function main() {
  ...
  log(`💡 Feed this file to your AI agent as context for WordPress API work.\n`);
}
main().catch((err) => {
...
async function main() {
  ...
  if (rootResult._error) {
    console.error(`❌ Could not reach REST API: ${rootResult._error}`);
    process.exit(1);
  }
  ...
  log(`\n💡 Commit this file and use Claude Code Web to generate typed mocks.\n`);
}
main().catch((err) => {
  console.error("Fatal error:", err);
  process.exit(1);
...
### Key Files
- `src/api/wp-schema.json` — Full REST API schema including ACF field definitions
  (generated via OPTIONS requests)
- `src/api/wp-sample-data.json` — Real sample responses from the API for each
  content type
...
<ColItem>**Never configure** Claude Code Web to call your WordPress API directly — always use the local-script-then-commit pattern</ColItem>
...
<ColItem>Make sure you're authenticated — ACF fields on some post types may require auth to see in OPTIONS responses</ColItem>
</SectionList>
