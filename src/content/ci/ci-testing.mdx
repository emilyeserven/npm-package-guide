...
<CIStepText>
<GoodTestsList>
  <li>**Independent** — no shared state between tests. Each test should set up its own data and clean up after itself.</li>
  <li>**Deterministic** — same result every time. Avoid dependencies on dates, random values, or network calls without mocking.</li>
  <li>**Fast** — especially unit tests. If your test suite takes minutes, developers stop running it.</li>
