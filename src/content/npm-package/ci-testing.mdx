...
<CIStepText>
<DefinitionTable termHeader="Quality" descHeader="Description">
  <DefRow term="Independent">No shared state between tests. Each test should set up its own data and clean up after itself.</DefRow>
  <DefRow term="Deterministic">Same result every time. Avoid dependencies on dates, random values, or network calls without mocking.</DefRow>
  <DefRow term="Fast">Especially unit tests. If your test suite takes minutes, developers stop running it.</DefRow>
  <DefRow term={<>Readable<FnRef n={3} /></>}>Someone new should understand what's being tested. Name tests clearly: <code>{"it('returns empty array when no items match filter')"}</code> is better than <code>{"it('test filter')"}</code>.</DefRow>
  <DefRow term={<>Behavior-focused<FnRef n={4} /></>}>Test what the code does, not how it does it. If you refactor internals, your tests should still pass.</DefRow>
</DefinitionTable>
