        ...
        <div key={step.num} className="step-card flex gap-4 py-4.5 border-b border-slate-200 dark:border-slate-700 relative last:border-b-0">
          ...
          <div className="flex-1 min-w-0">
            {step.jumpTo ? (
              ...
              >
                <span>{parse(step.title)}</span> <span className="text-blue-500 dark:text-blue-400">{'\u2192'}</span>
              </div>
            ) : (
              <div className="text-base font-bold text-slate-900 dark:text-slate-100 mb-1">{parse(step.title)}</div>
            )}
            <div className="text-sm text-slate-800 dark:text-slate-300 leading-relaxed mb-2">{parse(step.desc)}</div>
            <div className="step-detail text-xs text-gray-400 dark:text-slate-400 leading-relaxed bg-slate-50 dark:bg-slate-800 rounded-lg py-2.5 px-3.5 mb-2 border border-slate-100 dark:border-slate-700">{parse(step.detail)}</div>
            {step.substep && (
