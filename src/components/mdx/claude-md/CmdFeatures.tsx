...
import { CMD_FEATURES } from '../../../data/claudeMdData'
import type { CmdFeature } from '../../../data/claudeMdData'

export function CmdFeatures() {
  const isDark = useIsDark()

  return (
    <AccordionList<CmdFeature>
      items={CMD_FEATURES}
      renderHeader={(f) => (
          ...
          {f.code && (
            <div className="relative">
              <div className="absolute top-2 right-2 z-10">
                <CopyButton text={f.code} className="px-2 py-1 rounded text-[10px] font-mono border transition-colors cursor-pointer" />
              </div>
              <pre
