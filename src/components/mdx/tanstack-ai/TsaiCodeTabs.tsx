...
import { TSAI_CODE_EXAMPLES } from '../../../data/tsaiData'
import type { TsaiCodeTab } from '../../../data/tsaiData'

export function TsaiCodeTabs({ exampleId }: { exampleId: string }) {
  const isDark = useIsDark()
  const group = TSAI_CODE_EXAMPLES[exampleId]
  const [activeTab, setActiveTab] = useState<string>(group?.tabs[0]?.id ?? '')

  if (!group) return null

  const active = group.tabs.find((t: TsaiCodeTab) => t.id === activeTab) ?? group.tabs[0]
  ...
  return (
    <div className="mb-6">
      ...
      <div className="flex gap-0.5">
        {group.tabs.map((tab: TsaiCodeTab) => {
