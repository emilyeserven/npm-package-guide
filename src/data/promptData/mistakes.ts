    ...
    items: [
      {
        ...
        prompt: 'Use zero-indexed, exclusive upper bounds for all loops and array slices. Verify boundary values with +1/-1 checks.',
      },
      {
        ...
        prompt: 'Describe complex boolean conditions in plain English before coding. Include a truth table for conditions with 3+ variables.',
      },
      {
        ...
        prompt: 'Handle all edge cases: empty arrays, null, undefined, 0, NaN, and empty strings. Add guard clauses at function entry points.',
      },
      {
        id: 'toc-math-formula',
        ...
        prompt: 'Include the exact formula or reference algorithm for any math logic. Verify with at least two sample inputs and expected outputs.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        prompt: 'Only use packages I have explicitly listed. Verify any suggested package exists on npmjs.com with >1k weekly downloads before using it.',
      },
      {
        ...
        prompt: 'Use only current APIs for the specified framework versions. No deprecated methods or legacy patterns.',
      },
      {
        ...
        prompt: 'Target the specified language and runtime only. Do not use patterns, behaviors, or idioms from other languages.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        prompt: 'Keep solutions simple. No unnecessary abstractions, design patterns, or indirection layers. Prefer inline logic over premature extraction.',
      },
      {
        ...
        prompt: 'Provide complete, runnable code with all imports, exports, types, and error handling. No placeholders or TODOs.',
      },
      {
        ...
        prompt: 'Follow the existing project patterns, conventions, and technology choices documented in CLAUDE.md. Do not introduce alternative libraries or architectures.',
      },
      {
        ...
        prompt: 'Follow OWASP security best practices. Sanitize all user input. Never use dangerouslySetInnerHTML without DOMPurify.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        prompt: 'Use camelCase for variables and functions, PascalCase for components and types, kebab-case for CSS classes and file names.',
      },
      {
        ...
        prompt: 'Only add comments for complex logic or non-obvious decisions. No comments that restate what the code already expresses.',
      },
      {
        ...
        prompt: 'Follow the project\'s Prettier and ESLint configuration. Match existing code style for semicolons, trailing commas, and indentation.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        prompt: 'Include all referenced variables in useEffect dependency arrays. Use refs for values that should not trigger re-renders.',
      },
      {
        id: 'toc-hooks-rules',
        ...
        prompt: 'Call all hooks unconditionally at the top level of the component. Never place hooks inside conditions, loops, or nested functions.',
      },
      {
        id: 'toc-key-prop',
        ...
        prompt: 'Use stable, unique IDs as React list keys. Never use array index as key when the list can be reordered, filtered, or modified.',
      },
      {
        ...
        prompt: 'Use React Context or Zustand stores for cross-cutting concerns. Prefer component composition (children) over passing props through 3+ levels.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        prompt: 'Never render user-supplied HTML without sanitization. Use DOMPurify or a Markdown renderer with XSS protection enabled.',
      ...
      },
      {
        ...
        prompt: 'All secrets must be server-side only. Client env vars must use the framework-specific public prefix. Never commit .env files.',
      },
      {
        ...
        prompt: 'Store tokens in httpOnly cookies, not localStorage. Validate authentication server-side on every request. Never trust client-only auth checks.',
      ...
      },
      {
        ...
        prompt: 'Always use parameterized queries or an ORM. Never build SQL or NoSQL queries with string concatenation or template literals.',
    ...
    items: [
      {
        ...
        prompt: 'Use responsive units (rem, %, vw) and Tailwind breakpoints. Design mobile-first. Test at 320px, 768px, and 1280px widths.',
      },
      {
        ...
        prompt: 'Follow WCAG 2.1 AA. All images need alt text. All interactive elements must be keyboard-accessible with visible focus styles.',
      },
      {
        ...
        prompt: 'Use design system tokens for spacing (multiples of 4px), colors (palette only), and typography (defined text styles only). No arbitrary values.',
      },
      {
        ...
        prompt: 'Use a defined z-index scale: dropdown 10, sticky 20, modal 30, tooltip 40. Never use arbitrary large z-index values.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        prompt: 'Use Tailwind\'s built-in scale values. Only use arbitrary values (brackets) when no built-in equivalent exists.',
      },
      {
        ...
        prompt: 'Use Tailwind CSS v4 syntax only: CSS-first configuration with @theme, not tailwind.config.js. No v3 patterns.',
      },
      {
        ...
        prompt: 'Never construct Tailwind class names dynamically with template literals. Use a static mapping object for variant selection.',
      },
      {
        id: 'toc-tw-dark-mode',
        ...
        prompt: 'Every background, text, and border color must have a corresponding dark: variant. Dark palette: bg-slate-800, text-slate-200, border-slate-700.',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        prompt: 'Add CSRF tokens to all state-changing forms and API routes. Generate with crypto.randomUUID() per session and validate on every POST/PUT/DELETE.',
      ...
      },
      {
        ...
        prompt: 'Set sameSite: \'Strict\' (or \'Lax\'), httpOnly: true, and secure: true on all session cookies.',
        deepDivePageId: 'sec-csrf',
      },
      {
        ...
        prompt: 'Never use GET requests for state changes. All data mutations must use POST, PUT, or DELETE methods.',
        deepDivePageId: 'sec-csrf',
      },
    ],
  },
    ...
    items: [
      {
        ...
        prompt: 'Only suggest well-known packages with >1k weekly downloads. Verify the exact package name on npmjs.com before adding as a dependency.',
      ...
      },
      {
        ...
        prompt: 'Pin production dependencies to exact versions. Use a lockfile (pnpm-lock.yaml) and commit it to version control.',
        deepDivePageId: 'sec-supply',
      },
      {
        ...
        prompt: 'Add pnpm audit --audit-level=high to the CI pipeline. Block merges on known high-severity vulnerabilities.',
        deepDivePageId: 'sec-supply',
      },
    ],
  },
    ...
    items: [
      {
        ...
        prompt: 'Treat all LLM output as untrusted. Validate against an allowlist of permitted actions and require user confirmation for sensitive operations.',
      ...
      },
      {
        ...
        prompt: 'Wrap user-provided content in clear delimiters (<user_document>). Instruct the model to treat delimited content as data, never as instructions.',
        deepDivePageId: 'sec-prompt-injection',
      },
      {
        ...
        prompt: 'Apply least-privilege to LLM tool access. Grant only the minimum permissions needed for the task. A summarizer needs read-only access, not write.',
        deepDivePageId: 'sec-prompt-injection',
      },
    ],
  },
...
export const TESTING_MISTAKES: TestingMistake[] = [
  {
    ...
    prompt: 'Use waitForSelector or expect(locator).toBeVisible() instead of waitForTimeout. Never use arbitrary timeouts in tests.',
  },
  {
    context: 'e2e',
    ...
    prompt: 'Each test must set up its own data and tear it down. No test should depend on another test\'s state or execution order.',
  },
  {
    context: 'e2e',
    ...
    prompt: 'Use data-testid attributes for all test selectors. Never use CSS class names or DOM structure paths that break on refactors.',
  },
  {
    context: 'e2e',
    ...
    prompt: 'Use page.waitForResponse() or route.fulfill() to control API timing in tests. Never assume API calls complete before assertions.',
  },
  {
    context: 'unit',
    ...
    prompt: 'Test what the function returns or renders, not how it works internally. Assert on outputs, not on spy call counts.',
  },
  {
    context: 'unit',
    ...
    prompt: 'Only use snapshot tests for stable, small structures. Prefer explicit assertions over toMatchSnapshot() for complex components.',
  },
  {
    context: 'unit',
    ...
    prompt: 'Mock at system boundaries only: external services, APIs, and databases. Never mock the module under test or its direct collaborators.',
  },
  {
    context: 'unit',
    ...
    prompt: 'Include tests for rejected promises, thrown errors, and timeout scenarios. Every async function needs at least one failure path test.',
  },
  {
    context: 'unit',
    ...
    prompt: 'Use precise matchers (toBe, toEqual, toStrictEqual) over negated assertions. Verify that assertions actually test what you intend.',
  },
  {
    context: 'e2e',
    ...
    prompt: 'Use beforeEach/afterEach hooks to set up and tear down a clean test environment. Remove all test artifacts after each test.',
  },
]
